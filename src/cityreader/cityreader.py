...
class City:
  ...
  def __str__(self):
    s = f"{self.name}, {float(self.lat)}, {float(self.lon)}"
    return s
import csv
...
def cityreader(cities=[]):
  ...
  with open('cities.csv', 'r') as city_file:
    ...
    for column in reader:
        city = City(column[0],float(column[3]),float(column[4]))
        cities.append(city)
    return cities
...
lat1 = float(input('input first lat: '))
lon1 = float(input('input first lon: '))
lat2 = float(input('input second lat: '))
lon2 = float(input('input second lon: '))
...
print(cityreader_stretch(lat1, lon2, lat2, lon2))